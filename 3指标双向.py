...
# 人均可支配收入取对数
panel['人均可支配收入（万元）'] = (panel['人均可支配收入'] / 10000).clip(lower=1)

indep_vars = ['抚养比', '人均可支配收入（万元）', '信贷深度']
...
out_file = os.path.join(OUTPUT_DIR, "三指标_双向固定效应.xlsx")
...
with pd.ExcelWriter(out_file, engine='openpyxl') as writer:
    ...
    # 面板数据
    panel[['地区', '年份', 'PCA综合得分'] + indep_vars].to_excel(
